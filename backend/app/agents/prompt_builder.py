        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "你的发言内容，30-200字。严禁超过200字也不得少于30字。模拟真实玩家的语气，必须有实质内容（点名怀疑对象、给出理由、表态等），禁止只用\"过\"或\"就这样\"等敷衍短句。少于30字的发言会被系统直接拒绝。",
                    }
                },
                "required": ["text"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "你的遗言内容，30-200字。必须包含你最后的分析、怀疑对象和给好人的建议。不得少于30字，禁止只用\"过\"或\"没话说\"敷衍。",
                    }
                },
                "required": ["text"],
            },
...
- **发言字数要求（极其重要）**：每次发言至少30字，必须有实质性内容。严禁以下行为：
  - **严禁**只说"过"、"pass"、"没话说"、"就这样"等敷衍性短句——这等于直接暴露你是AI或放弃游戏
  - **严禁**发言少于30个汉字——这是最低限度，少于30字的发言会被系统拒绝
    ...
    def _task_instruction(
        ...
        if context == "witch_poison":
            return (
                ...
                "- **检查已出局玩家**：查看上方「当前游戏状态」中列出的「已出局玩家」。这些都是已经死了的人。\n"
                "- **毒杀已出局玩家 = 浪费毒药**：毒药对已死玩家无效，毒死人不会产生任何效果，但你的毒药会永久消失。\n"
                "- **合法目标只能是存活玩家**：你的毒杀目标必须在「存活玩家」列表中。\n\n"
                "### 第三步：做出毒杀判断\n"
                "基于第一步的深度分析和第二步的验证，从**存活玩家**中选择值得毒杀的目标：\n\n"
                "**你应该积极寻找值得毒杀的目标：**\n"
                ...
                "**以下情况不要毒**：仅仅因为某人让你不爽或踩了你——这不是理由；信息不足且没有明确的逻辑指向时也不要盲目出手。**绝对不能毒杀已出局玩家——毒死人是纯粹的浪费。**\n\n"
                "**你的毒药是用来用的，不是用来攒到游戏结束的。如果你经过深度分析后有明确的怀疑对象（且确认目标存活），今晚就是最好的下手时机。**\n\n"
                '请严格按JSON格式输出：{"action_type":"poison","target_seat":<目标>,"reasoning":"<基于深度分析的毒杀理由，不少于30字>"} 表示毒人，\n'
        ...
        if context == "day_speech":
            ...
            return (
                ...
                "   - 发言要有个性：用自己的语言，不要套模板\n"
                "   - 发言不超过200字，但要有实质内容\n"
                "   - **发言至少30字！少于30字的发言会被系统拒绝！只说\"过\"或\"就这样\"是绝对不可接受的！**\n"
