        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "你的遗言内容，15-200字。必须包含你最后的分析、怀疑对象和给好人的建议。禁止只用\"过\"或\"没话说\"敷衍。",
                    }
                },
                "required": ["text"],
            },
...
- **夜晚死亡（狼刀、毒杀、猎人带人）**：天亮时系统公告"X号玩家死亡"，但**不公布其身份**。只有女巫通过刀口信息知道谁被狼刀，其他玩家只知道有人死了但不知道他的身份
- **放逐出局**：被投票放逐的玩家**公开翻开身份牌**，所有人都能看到他的真实身份
- 出局玩家不能再参与发言和投票
...
- **🐺 狼队频道（极其重要）**：夜晚刀人阶段的投票理由是一个**仅狼人可见的私密频道**。好人、神职完全看不到这个频道的任何内容。你和狼队友在这里的交流是绝对安全的——**不需要任何掩饰，不需要伪装成好人，直接说真话、讲策略、暴露你的真实意图**。你可以直接点名"我怀疑X号是预言家，今晚刀他"，也可以直接讨论"明天我们要不要卖Y号"。这个频道的发言永远不会泄露给狼人以外的任何人。
- **玩法要点**：
  - 白天伪装成好人融入推理；对狼队友适度踩一下避免绑在一起；不要所有狼人站同一队；不要沉默
  - **主动编造故事线**：你是反派主角，要创造剧情。编造逻辑攻击好人、制造对立、带节奏推人
  - **狼队友配合**：夜晚投票的理由就是你们的狼队频道，完全私密。在这里你可以开诚布公地说出你的真实想法——不用伪装、不用绕弯子。直接告诉队友你建议刀谁、为什么、明天白天打算怎么演。同时参考队友的理由来协调你们的整体策略。
...
- **⚠️ 亮明身份的决策铁律（极其重要！）**：你是好人阵营的信息核心，**你的存活时间直接决定好人能获得多少轮查验信息**。亮明身份前必须完成以下深度分析：
  1. **总结所有查验信息**：你查了几个人？分别是什么结果？这些结果之间有没有逻辑关联？
  2. **推算场上局势**：已知翻牌身份有哪些？还剩几个狼？还剩几个神职？你的金水/查杀在当前的狼坑中处于什么位置？
  3. **评估报信息的紧迫性**：你查到的信息是好人现在必须知道的（如查杀了一个正在带节奏的狼人），还是可以暂时等等再报的？
  4. **权衡跳身份的代价**：你跳了预言家 = 狼人当晚大概率刀你 = 查验链条中断。好人少一轮查验信息可能会输。
  **什么时候应该跳**：你查到了查杀（狼人）且今天可能推错好人；场上信息极度混乱需要你来带方向；你存活轮次已经不多了。
  **什么时候不该跳**：你的信息不是当场急需的（等一等可以更准确地再查一人）；场上已经有另一个预言家在带节奏（先判断他是真是假）；你判断跳了也改变不了今天的投票结果。
...
- **玩法要点**：首夜建议用解药保轮次；藏好身份——女巫是狼人优先击杀目标；银水（被救者）不一定是好人（狼可能自刀骗药）
- **🐺 毒药使用指南**：毒药是好人阵营最强的主动击杀手段——**你有责任在合适的时机果断使用它**。使用毒药前**必须深度思考、总结所有信息、再做出判断**，不能凭感觉或冲动行事。以下情况**应该积极考虑毒杀**：
  - 某玩家发言逻辑严重矛盾，且投票行为一致可疑
  - 预言家报出查杀，且你倾向于相信该预言家
  - 某玩家发言中出现明显的狼人视角（知道不该知道的信息、从狼人阵营角度思考问题）
  - 某玩家连续多轮带节奏推好人、制造混乱，明显在帮狼人阵营
  - 进入中后期，解药已用，你有明确的狼人怀疑对象——果断下手
- **⚠️ 毒杀目标合法性验证（极其重要！）**：毒药只能对**存活**玩家使用。已出局的玩家已经死了，毒杀死人 = 白白浪费毒药 = 帮狼人减少轮次。使用毒药前**必须检查**：你的目标是否在已出局玩家列表中？凡是已出局（含被放逐、被狼刀、被毒、被猎人带走的）的玩家，**绝对不能**选为毒杀目标。毒杀死人不会产生任何效果，但你的毒药会永久消失。
...
- **发言字数要求**：每次发言至少15字，必须有实质性内容。严禁以下行为：
  ...
  - **严禁**发言少于15个汉字——这是最低限度
  - 如果你真的无话可说，至少要说清楚**为什么**你觉得无话可说，并点名你的怀疑对象
...
- **女巫特别提醒**：毒药是你的武器，不是摆设。使用毒药前**必须深度思考、总结所有信息、再做判断**——回顾全场发言、分析投票行为、梳理翻牌身份和查验结果，用完整的信息链支撑你的毒杀决定。犹豫不决、到死都不用毒药，等于白费了这张王牌。但也不要盲目出手——没有经过深度分析、仅凭感觉和冲动毒人反而害了好人阵营。与其留着毒药到游戏结束，不如在你有明确怀疑对象时果断出手。
...
{self._format_conversations(conversation_log, state.round_number, seat, role_name)}"""
    ...
    def _task_instruction(
        ...
        if context == "night_kill":
            ...
            if mates:
                prior_hint = (
                    "**请查看上方「本轮对话记录」中[狼队频道]的消息，了解队友的刀人建议。**\n"
                    "**注意：[狼队频道]是绝对私密的，只有你和队友能看到，好人完全不知道你们的交流。**\n"
                    "你的理由文字就是在狼队频道的发言——直接说出你的真实想法，不需要掩饰。\n"
                    "参考队友的意见来决定刀人目标，但**队友的判断不一定对**，请独立分析。\n"
                    "如果你第一个投票，请给出完整的形势分析和刀人建议，帮队友理解你的判断。"
                )
            return (
                f"## 你的任务：夜晚刀人\n"
                f"你是狼人，{mate_info}\n"
        ...
        if context == "witch_poison":
            ...
            return (
                ...
                "### 第一步：深度思考与信息总结（必须完成）\n"
                "在做出毒杀决定之前，你**必须**先在内心完成以下深度分析：\n\n"
                "- **回顾全场发言**：逐一梳理每位存活玩家的发言内容，谁说了什么？谁的逻辑前后矛盾？\n"
                "- **分析投票行为**：回顾每一轮投票，谁的票型和发言一致？谁言行不一？\n"
                "- **梳理已知信息**：死亡玩家翻牌身份是什么？预言家报了哪些查验？女巫救过人没有？把这些已知信息串成一条链。\n"
                "- **构建狼坑**：基于以上所有信息，列出你心中的狼坑（至少2-3个怀疑对象），并说明每个人被怀疑的**具体原因**。\n"
                "- **排除法验证**：用翻牌身份和查验结果反向验证——有没有人可以排除嫌疑？你的狼坑里有没有矛盾？\n"
                "- **换位思考**：如果你是一只狼，在当前的局势下你会怎么投票、怎么发言？谁的行为最符合狼人的利益？\n\n"
                "### 第二步：验证目标合法性（必须完成）\n"
                "在确定毒杀目标之前，你**必须**执行以下验证：\n\n"
                f"- **存活玩家是：{alive_str}** —— 你只能从这些人中选择目标。\n"
                f"- **已出局玩家是：{dead_str}** —— 这些人已经死了，毒他们无效。\n"
                "- **毒杀已出局玩家 = 浪费毒药**：毒药对已死玩家无效，毒死人不会产生任何效果，但你的毒药会永久消失。\n"
                "- **在输出 target_seat 之前，请大声问自己：这个号码在存活玩家列表中吗？如果不在，立即换目标！**\n\n"
                "### 第三步：做出毒杀判断\n"
                f"基于第一步的深度分析和第二步的验证，从**存活玩家（{alive_str}）**中选择值得毒杀的目标：\n\n"
                "**你应该积极寻找值得毒杀的目标：**\n"
                "- 回顾白天的发言，谁在带节奏推好人？谁的逻辑前后矛盾？谁的投票和发言对不上？\n"
                "- 谁在发言中表现出狼人视角——知道不该知道的信息、从狼人阵营角度思考？\n"
                "- 有没有人连续几轮都在制造混乱、引导大家怀疑好人？\n\n"
                "**以下情况果断毒杀**：经过深度分析后有明确怀疑对象且能说出逻辑理由；预言家查杀且你相信该预言家；某人发言有明显狼人视角。\n"
                "**以下情况不要毒**：仅仅因为某人让你不爽或踩了你——这不是理由；信息不足且没有明确的逻辑指向时也不要盲目出手。\n"
                f"**绝对不能毒杀已出局玩家——毒死人是纯粹的浪费。你的目标必须在 {alive_str} 之中！**\n\n"
                "**你的毒药是用来用的，不是用来攒到游戏结束的。如果你经过深度分析后有明确的怀疑对象（且确认目标存活），今晚就是最好的下手时机。**\n\n"
                '请严格按JSON格式输出：{"action_type":"poison","target_seat":<目标>,"reasoning":"<基于深度分析的毒杀理由，不少于15字>"} 表示毒人，\n'
                '或 {"action_type":"pass","reasoning":"<不用的理由>"} 表示放弃毒药。'
            )
        ...
        if context == "day_speech":
            ...
            return (
                ...
                "**如果你是预言家，特别注意**：跳明身份之前必须完成深度分析——你查过谁、结果是什么、现在报信息是否必要、跳了会不会当晚被刀。预言家的命比单条信息重要，不要因为想说话就暴露。只有查杀到狼人、或场上信息极度混乱需要你来带方向时，才考虑亮身份。\n"
