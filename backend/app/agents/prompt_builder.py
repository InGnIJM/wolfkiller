...
### 毒药使用指南
毒药是好人阵营最强的主动击杀手段——**你有责任在合适的时机果断使用它**。使用毒药前**必须深度思考、总结所有信息、再做出判断**，不能凭感觉或冲动行事。
...
犹豫不决、到死都不用毒药，等于白费了这张王牌。但也不要盲目出手——没有经过深度分析、仅凭感觉和冲动毒人反而害了好人阵营。与其留着毒药到游戏结束，不如在你有明确怀疑对象时果断出手。"""
    ...
    def _task_instruction(
        ...
        if context == "night_kill":
            ...
            return (
                ...
                f"- **thinking 字段**：在内心进行深度分析——当前局势如何、每个存活玩家的身份可能、刀谁对狼队最有利、风险与收益权衡。思考过程只有你自己能看到，其他玩家看不到\n"
                f"必须严格按JSON格式输出：{{"
                f'"thinking":"<你的完整思考分析过程，不低于30字>",'
        ...
        if context == "witch_save":
            ...
            return (
                ...
                f'请严格按JSON格式输出：{{"thinking":"<你的思考分析>","action_type":"save","reasoning":"<理由>"}} 表示救人，\n'
                f'或 {{"thinking":"<你的思考分析>","action_type":"pass","reasoning":"<理由>"}} 表示不救。'
            )
        ...
        if context == "witch_poison":
            ...
            return (
                ...
                "- **梳理已知信息**：预言家报了哪些查验？女巫救过人没有？出局玩家生前的发言和投票有什么规律？把这些已知信息串成一条链。\n"
                "- **构建狼坑**：基于以上所有信息，列出你心中的狼坑（至少2-3个怀疑对象），并说明每个人被怀疑的**具体原因**。\n"
                "- **排除法验证**：用已知的发言、投票和查验结果反向验证——有没有人可以排除嫌疑？你的狼坑里有没有矛盾？\n"
                ...
                '请严格按JSON格式输出：{"thinking":"<你的深度分析思考过程，不低于30字>","action_type":"poison","target_seat":<目标>,"reasoning":"<基于深度分析的毒杀理由，不少于15字>"} 表示毒人，\n'
                '或 {"thinking":"<你的思考分析>","action_type":"pass","reasoning":"<不用的理由>"} 表示放弃毒药。'
            )
        ...
        if context == "night_check":
            ...
            return (
                ...
                '必须严格按JSON格式输出：{"thinking":"<你的完整思考分析，不低于30字>","action_type":"check","target_seat":<座位号>,"reasoning":"<你的推理，不低于15字>"}'
            )
        ...
        if context == "hunter_shoot":
            return (
                ...
                "如果你决定开枪，请选择目标。\n"
                '必须严格按JSON格式输出：{"thinking":"<你的完整思考分析，不低于30字>","action_type":"shoot","target_seat":<目标>,"reasoning":"<推理>"}'
                ' 或 {"thinking":"<你的思考分析>","action_type":"pass","reasoning":"<理由>"} 表示不开枪。'
            )
        ...
        if context == "exile_vote":
            return (
                ...
                '必须严格按JSON格式输出：{"thinking":"<你的完整思考分析，不低于30字>","target_seat":<目标座位号或0>,"reasoning":"<你的推理>"}'
            )
