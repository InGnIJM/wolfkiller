...
def _utf8(value: str, name: str, *, token: bool = False) -> str:
    ...
    if not value or len(value) > 256 or (token and _TOKEN.fullmatch(value) is None):
...
def _validate_payload(effect: GameEffect, seats: set[int]) -> dict[str, object]:
    ...
    if kind is EffectKind.ACCEPT_ACTION:
        if effect.target_seat is not None:
            raise EffectRejected("accept action cannot have target")
        return payload
    if kind is EffectKind.EMIT_EVENT:
        _token_field(payload, "event_type")
        if not isinstance(payload["payload"], Mapping):
            raise EffectRejected("event payload must be a mapping")
        if effect.target_seat is not None and effect.target_seat not in seats:
    ...
    if kind in {EffectKind.CONSUME_RESOURCE, EffectKind.SET_RESOURCE}:
        ...
        if not isinstance(payload["fact"], Mapping):
            raise EffectRejected("fact must be a mapping")
...
def _check_preconditions(effect: GameEffect, runtime: _Runtime, alive: dict[int, bool]) -> None:
    ...
    if not set(effect.preconditions) <= allowed:
        raise EffectRejected("unknown precondition")
    ...
    if "resource_equals" in effect.preconditions:
        ...
        if not isinstance(condition, Mapping):
            raise EffectRejected("resource precondition invalid")
        ...
        if target is None or runtime.role_resources.get(target, {}).get(resource, 0) != value:
            raise EffectRejected("resource precondition failed")
    if "private_equals" in effect.preconditions:
        condition = effect.preconditions["private_equals"]
        if not isinstance(condition, Mapping):
            raise EffectRejected("private precondition invalid")
        _exact(condition, frozenset({"key", "value"}), "private precondition")
        key = _token_field(condition, "key")
        if target is None or runtime.private_data.get(target, {}).get(key) != condition["value"]:
            raise EffectRejected("private precondition failed")
    if "status_present" in effect.preconditions:
        ...
        if actual is not present:
            raise EffectRejected("status precondition failed")
def _apply_one(effect: GameEffect, payload: dict[str, object], runtime: _Runtime,
    ...
    if kind is EffectKind.SET_RESOURCE:
        ...
        if resources.get(resource, 0) < amount:
            raise EffectRejected("resource underflow")
        resources[resource] -= amount
    elif kind is EffectKind.SET_PRIVATE_DATA:
        runtime.private_data.setdefault(target, {})[str(payload["key"])] = payload["value"]
    elif kind in {EffectKind.ADD_STATUS, EffectKind.REMOVE_STATUS}:
        statuses = runtime.statuses.setdefault(target, set())
        status = str(payload["status"])
        if kind is EffectKind.ADD_STATUS:
            if status in statuses:
                raise EffectRejected("status already present")
            statuses.add(status)
        else:
            if status not in statuses:
                raise EffectRejected("status is absent")
        ...
        if kind is EffectKind.ADD_RELATION:
            if relation in relations:
                raise EffectRejected("relation already present")
            relations.add(relation)
        else:
            if relation not in relations:
                raise EffectRejected("relation is absent")
        ...
        if not alive[target]:
            raise EffectRejected("player already dead")
        alive[target] = False
...
class EffectApplier:
    ...
    def _apply_locked(self, state: GameState, effects: tuple[GameEffect, ...],
        ...
        if type(permission) is not EffectPermission:
            raise TypeError("permission must be EffectPermission")
        if not effects or any(type(effect) is not GameEffect for effect in effects):
            raise EffectRejected("effects must contain GameEffect values")
        action_keys = {effect.source_action_key for effect in effects}
        if len(action_keys) != 1:
            raise EffectRejected("all effects must have one action key")
        ...
        if len(ids) != len(set(ids)):
            raise EffectRejected("duplicate effect id")
        if sum(effect.kind is EffectKind.ACCEPT_ACTION for effect in ordered) != 1:
            raise EffectRejected("batch must contain exactly one accept action")
        for ordinal, effect in enumerate(ordered):
            if effect.effect_id != derive_effect_id(action_key, ordinal):
            ...
            if len(effect.visibility) != len(set(effect.visibility)) or not set(effect.visibility) <= permission.allowed_visibility:
                raise EffectRejected("visibility permission denied")
        ...
        simulated.events += tuple(_freeze(event) for event in generated_events)
